...
export class WishListService {
    ...
    this.getAllFromWishList(wishListId).subscribe((wishListData) => {
      if (wishListData != null) {
        ...
        if (existingItemIndex !== -1) {
          ...
          console.log(
            `Item exists, new quantity: ${wishList[existingItemIndex].quantity}`
          );
        } else {
  ...
  setToWishList(wishListId: string, wishListItems: any) {
    ...
    return this.http
      .post<any>(
        `https://localhost:7283/api/Wishlist/SetWishlist?wishListId=${wishListId}`,
        { id: wishListId, items: wishListItems }
      )
      .pipe(
        catchError((error) => {
  ...
  getAllFromWishList(wishListId: string): Observable<any> {
    return this.http
      .get<any>(
        `https://localhost:7283/api/Wishlist/GetWishlistById/${wishListId}`,
  ...
  deleteWishList(wishListId: string) {
    return this.http
      .delete<any>(
        `https://localhost:7283/api/Wishlist/DeletewishList/${wishListId}`
      )
      .pipe(
        catchError((error) => {
  ...
  removeWishList(wishListId: string) {
    this.deleteWishList(wishListId).subscribe((p) => {
      this.wishListProductSource.next([]);
